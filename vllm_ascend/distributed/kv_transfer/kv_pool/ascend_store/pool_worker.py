...
class KVPoolWorker:
    ...
    def __init__(
        ...
        # Prefetch window: how many layers ahead to submit async KV pool reads.
        # RDMA read latency (~50ms) / per-layer compute (~3ms) ≈ 16 layers ahead
        # needed to fully hide transfer latency behind computation.
        self.prefetch_window = vllm_config.kv_transfer_config.kv_connector_extra_config.get(
            "prefetch_window", max(self.num_kv_buffer_layers * 4, 16)
        )
    ...
    def _init_prefetch(self, metadata: AscendConnectorMetadata) -> None:
        ...
        for buf_idx in range(self.num_kv_buffer_layers):
            if (self.buffer_pending_counts is None
                    or self.buffer_pending_counts[buf_idx] == 0):
    ...
    def _load_layer_kv(self, connector_metadata: AscendConnectorMetadata, layer_id: int) -> None:
        ...
        if future is not None:
            # Block until prefetch completes to avoid data race:
            # async m_store.get writes to the buffer in a background
            # thread; a parallel sync m_store.get would corrupt data.
            future.result()  # wait for async prefetch to finish
        ...
        self._load_layer_kv_sync(connector_metadata, layer_id)

    def _load_layer_kv_sync(self, connector_metadata: AscendConnectorMetadata, layer_id: int) -> None:
        """同步加载KV cache"""
        for request in connector_metadata.requests:
            load_spec = request.load_spec
            if load_spec is None or not load_spec.can_load:
    ...
    def start_load_kv(self, metadata: AscendConnectorMetadata):
        ...
        # Lazy-init counter/condition for K-layer buffering and share with send thread.
        if self.is_klayer_buffering_enabled() and self.buffer_pending_counts is None:
        ...
        # Per-chunk timing
        _now = time.time()
        _gap = (_now - self._last_chunk_time) * 1000 if hasattr(self, '_last_chunk_time') else 0
        self._last_chunk_time = _now
        if _gap > 0:
            logger.info(
                "Chunk gap: %.1f ms since last start_load_kv, _has_load=%s, num_reqs=%d",
                _gap, self._has_load_requests, len(metadata.requests),
            )
        ...
        if self.is_klayer_buffering_enabled():
            for request in metadata.requests:
                logger.info(
                    "Late lookup check: req_id=%s, load_spec=%s, block_hashes=%s",
                    request.req_id,
                    request.load_spec,
                ...
                for start, end, key in self.token_database.process_tokens(
                    ...
                    # Use LayerPoolKey format to match layerwise store keys.
                    # The layerwise store saves KV as model@...@chunk_hash@layer_id,
                    # but PoolKey.to_string() lacks the @layer_id suffix.
                    # Check layer 0 as representative — all layers of a block
                    # are stored together in the same forward pass.
                    layer_keys = key.split_layers(self.num_layers)
                    lookup_keys.append(layer_keys[0].to_string())
                if not lookup_keys:
                    continue
                ...
                if kvpool_cached >= self.block_size:
                    ...
                    logger.info(
                        "Late lookup: request %s found %d tokens in pool, can_load=True",
                        request.req_id, kvpool_cached,
                    )
                else:
                    request.load_spec = None
                    logger.info(
                        "Late lookup: request %s no tokens in pool (< %d), cleared load_spec",
                        request.req_id, self.block_size,
                    )
        ...
        if load_start_time is not None:
            ...
            if self.current_layer % 10 == 0:
                logger.info(
                    ...
                    f"load_time={load_time:.2f}ms, num_buffers={self.num_kv_buffer_layers}, "
                    f"prefetch_pending={len(self._prefetch_futures)}"
                )
    ...
    def get_and_clear_finished_requests(self, finished_req_ids, meta: AscendConnectorMetadata) -> set[str]:
        if self.use_layerwise:
            ...
            # Gate by finished_req_ids to align with non-layerwise behavior:
            # only report finished_sending for requests the scheduler has
            # already marked as finished.  Otherwise _update_from_kv_xfer_finished
            # will call _free_blocks on an unfinished request and assert-fail.
            all_done = self.kv_send_thread.get_and_clear_finished_requests()  # type: ignore[union-attr]
            finished_sending = {r for r in all_done if r in finished_req_ids}
            for r in all_done:
